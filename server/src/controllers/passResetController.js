...
const { User, PasswordRes } = require("../db");
  ...
  try {
    ...
    const mailOptions = {
      ...
      html: `
                ...
                <p>Atentamente,</p>
                <p><img src="https://res.cloudinary.com/dclvhbrj3/image/upload/v1719355309/logo_fefe_bakes_lrwkjb.jpg"></p>`,
    };
...
const resetPassword = async (req, res) => {
  try {
    ...
    const resetToken = await PasswordRes.findOne({ where: { token } });
    ...
    res
      .status(200)
      .json({ success: true, message: "Contraseña restablecida con éxito" });
  } catch (error) {
    console.error(error);
    res.status(500).json({ success: false, message: "Error en el servidor" });
  }
